...
// hex values pass through; var(--…) values FALL BACK to the scheme's defaults with a dev-only warn (CSS variables don't exist in RN — documented limitation); dark resolution mirrors web (theme.dark?.x ?? THEME_DARK_DEFAULTS.x); null theme → pure defaults; fontFamily: SAME precedence as the web resolver (src/react/theme.ts resolveFontFamily — fontFamily FIRST, then pattern-gated googleFont): take the winning value's first comma-separated family, strip quotes, for RN's single-family model — else null (system font).
```
