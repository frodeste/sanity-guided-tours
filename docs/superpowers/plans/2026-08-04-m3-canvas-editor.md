...
Behavior: vertical list grouped by chapter (chapter title header with add-step + add-chapter buttons); step rows show thumbnail (screenshot CDN url + `?w=160`), title/index, element-count badge, validation warning marker (from `props.validation`/member validation if cheaply available — else a presence-of-required-fields check: screenshot missing **or `screenshot.alt` missing/empty** → warning, so bulk-uploaded steps — which scaffold without alt by construction — surface as needing attention; record which mechanism was used). Selection highlight. Per-step menu (`MenuButton`): Duplicate (duplicateStepPatch with regenerated keys), Delete (removeStepPatch with confirm dialog), Move to chapter ▸ (list of other chapters → moveStepPatch append). Reorder within a chapter via up/down menu items + drag (HTML5 draggable with `insert`-based move patches); cross-chapter moves via the menu only (drag-across is a stretch, skip — record as deferred).
...
Behavior:
- Drop zone on the filmstrip (chapter header or footer target): drag-over highlight; on drop of image files → `filesInUploadOrder` → **strictly sequential** uploads: a for-await loop, one `client.assets.upload('image', file)` in flight at a time (`useClient({apiVersion: '2026-08-01'})`), each await wrapped in try/catch so one failure never aborts the batch (collect results as settled-shaped records for `partitionResults`); the `n/m` progress text increments per completed file. Sequential is deliberate: screenshot batches are small, order maps to progress intuitively, and it avoids hammering the assets endpoint. Then a single `PatchEvent` inserts all successful `stepsFromAssets` scaffolds → toast (useToast) reporting ok/failed counts. Also an "Upload screenshots…" button (file input) for keyboard/no-drag users.
